...
// vue/define-macros-order 要求宏（defineProps/defineEmits）先于其它语句；
// 因此 withDefaults 默认值只能用字面量，不能引用下方常量（否则 TDZ 运行时报错），
// duration/easing 的字面量须与 DEFAULT_DURATION/DEFAULT_EASING 保持一致
...
// 常量定义（供运行时逻辑引用；withDefaults 默认值已内联同值字面量）

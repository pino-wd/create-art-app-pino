...
import { useChartComponent } from '@/hooks/core/useChartComponent'
...
// 使用新的图表组件抽象（需先于 generateChartOptions 等使用其解构结果的函数声明，
// no-use-before-define 对 const 解构变量按源码顺序判定）
const {
  chartRef,
  initChart,
  getAxisLineStyle,
...
function initAnimationData(): number[] | LineDataItem[] {
  if (isMultipleData.value) {
    ...
    return multiData.map(item => ({
      ...item,
      // Array.from 第二参数显式映射为 number，避免 fill(0) 推导出 unknown[]
      data: Array.from({ length: item.data.length }, () => 0),
    }))
  }
  const singleData = props.data as number[]
  return Array.from({ length: singleData.length }, () => 0)
}
...
// chartRef 仅通过模板字符串 ref（ref="chartRef"）由运行时绑定，
